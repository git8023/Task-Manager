 ...
 * @param d 目标日期
 * @param type 返回值类型, 默认返回毫秒数. "MS", "HH", "DAY"
 */
 ...
 */
String.prototype.notEquals=function(other, isIgnoreCase, isTrim){return !this.equals(other, isIgnoreCase, isTrim);}

/**
 * 截取尾部字符串
 * @param maxLen 字符串最大长度
 * @param suffix 截取后添加的后缀, 当前值只获取字符串类型
 * @returns {String} 当this.length&lt;=maxLen 时,原样返回; 否则,返回值:当前字符串截取0~maxLen长度+suffix
 */
...
Array.prototype.unique = function() {
	...
	for (var i = 0, len = this.length, val; i < len; i++)if (!tmp[val = this[i]])target.push(val) && (tmp[val] = true);
	this.length = 0;
	for (var i=0;i<target.length;i++)this.push(target[i]);
	return this;
};

/**
 * 遍历数组
 * @param callback 回调函数, 参数:v-[value],i-[index]
 ...
 * @param byIndex 是否通过索引匹配, true-元素序列号必须一致(默认), false-包含即可但长度必须一致
 */
...
/**
 * 数组过滤
 * @param p {Function} predicate 断言测试函数, 返回值:true-保留, 参数:元素值
 * @returns {Array} 被排除元素列表
 */
Array.prototype.filter = function(p) {
	if (0 >= this.length) return;
	var incEl=[],excEl=[],_this=this;
	this.each(function(v,i){if(true==p(v)){incEl.push(v);}else{excEl.push(v);}});
	this.length=0;
	incEl.each(function(v,i){_this.push(v);});
	return excEl;
};

/**
 * 追加元素
 * @param el {Object|Array} 被追加元素(或数组)
 */
Array.prototype.append = function(el) {
	var _this = this;
	if(el instanceof Array) el.each(function(v,i){_this.push(v);}); 
	else _this.push(el);
	return _this;
}
...
/**
 * 匹配指定值是否在数组中存在
 * @param target {Object} 目标值
 * @param matcher {Function} 匹配器, 未定义时使用全等匹配(===); 参数:originalValue, targetValue; 返回值:true-已找到匹配元素;
 * @returns {Number} 元素在数组中的下标, -1不包含指定元素
 */
Array.prototype.match = function(target, matcher) {
	if(!validator.isFunction(matcher)) matcher=function(origV, tV) {return origV===v;}
	var idx=-1;
	this.each(function(v,i){if(true==matcher(v,target)){idx=i;return false;}});
	return idx;
}

/**
 * 分段处理数组
 * @param size -{Number} 每次处理的最大长度
 * @param handler -{Function} 分段处理器
 * @param timer -{Number} 指定setInterval间隔时长, 是否需要使用 setInterval 处理
 */
Array.prototype.limit = function(size, handler, timer) {
	...
	if (0 == arr.length) {clearInterval(_timer_Id_);return;}
	handler(arr);
	count++;
...
if ($) {
	/**
	 * 禁用控件
	 * @param countdown {Number} 禁用倒计时, x秒后启用当前控件. 接受正整数.
	 */
	 ...
	 */
	$.fn.getHTML = function() {return $("<div/>").append($(this).clone()).html();};
	
	/**
	 * 限时禁用指定控件
	 * @param timer 倒计时(ms), 默认值3000ms
	 */
	$.fn.limitedDisplay = function(timer) {timer=parseInt(timer);timer=(0>=timer?3000:timer);var $this=$(this).attr("disabled","disabled");setTimeout(function(){$this.removeAttr("disabled")}, timer); return $this;};
	
	/**
	 * 为select控件设置option列表
	 * @param data {Array} option数据, eg:[{value:String, text:String}, {value:String, text:String}, ...], eg2:[String, String, ...]
	 * @param isAppend {Boolean} true-追加在原来的数据后面, false-清理原来的数据(default)
	 * @param value {String} select选中的值
	 */
	$.fn.setOptions=function(data, isAppend, value){
		...
		if (data instanceof Array){
			data.each(function(v){
				if ("string".equals(typeof v, true, true)){v={value:v, text:v}};
				$("<option/>",{value:v["value"], html:v["text"]}).appendTo($this);
			});
