...
function IssueFn() {
	...
	this.initEvents = function() {
		...
		baseCtnr.undelegate(".issues .issue","click").delegate(".issue_list .issues .issue", "click", showDetailsEvent);
	...
	function exportWordEvent(){
		...
		requestUtil.download({url:"issue/exportWord.cmd", param:{issueId:issueId}});
	}
	...
	function deleteCurrentIssueEvent() {
		...
		if (currentIssueId) {
			$.jc.confirm("Info", "Will delete the specified issue["+$.red(getCurrentIssueName()).getHTML()+"], Are you sure continue?", function(){
				requestUtil.jsonAjax({
					url:"issue/removeIssue.cmd",
					data:{issueId:currentIssueId},
	...
	function clearIssueEvent() {
		...
		$.confirm({
			...
			confirm:function(){
				if (0 < issueIds.length) {removeAll();}
				else {$.jc.warning("Cannot found any issue");}
			}
		});
	}
	...
	function maxOrMinFullDetailEvent() {
		...
		var fulled=$this.attr(FULLED_DETAILS);
		if (fulled) {
		...
		detailCtnr.animate({
			...
			if (!fulled) {$(".note-editable.panel-body").height(320); labelCls="col-sm-2 control-label"; iptCls="col-sm-10";}
	...
	function showDetailsEvent() {
		...
		requestUtil.jsonAjax({
			...
			data : {id:getCurrentIssueId()},
			success : function(rData){
				if (!rData.flag){$.jc.warning(rData.message); return;}
				defailForm.fillForm({data:rData.data});
	...
	function initIssues() {
		...
		requestUtil.jsonAjax({
				...
				if (issuesData instanceof Array) {
					if (0 != issuesData.length) {
						...
						issuesData.each(function(issue){
							$("<li/>",{issueId:issue["id"], html:issue["name"]})
								.addClass("issue")
								.appendTo(issuesCtnr);
						});
			...
			error : function(){$.jc.warning();}
		});
	}

	// 初始化富文本编辑器
	function initHtmlEditors() {
		var SUMMER_NOTE_CONF={toolbar:[['view', ['fullscreen', 'codeview', 'help']]]};
	...
	// 释放富文本编辑器
	function destroySummerNote() {getDetailForm().find("textarea[name].form-control").each(function(i,txt){$(txt).summernote("destroy");initTextarea($(txt));});}
	...
	function enabledMenusForAIssue(){
		if(getCurrentIssueId()){baseCtnr.find(".menus .menu").removeClass(DISABLED);}
		else{baseCtnr.find(".menus .menu").addClass(DISABLED); baseCtnr.find(".menus .menu.add").removeClass(DISABLED);}
	}
	...
	function enabledDetails(){
		...
		detailForm.find("input[name='createdBy']").attr({"disabled":"disabled"}).val($("#account").text().trim());
		detailForm.find("input[name='createdDate']").attr({"disabled":"disabled"}).val(new Date().format("yyyy-MM-dd hh:mm"));
	...
	function disabledDetails() {
		var detailForm=getDetailForm();
		detailForm.find(".form-control").attr({"disabled":"disabled"});
	...
	function showTxtDisplayContainer(isShow) {
		getDetailForm().find("textarea[show-container]").each(function(i, txt) {
			...
			if (isShow) {ctnr.show(); txt.hide();} 
			else {txt.show(); ctnr.hide();}
		});
	}
	
	function getDetailForm(){return getDetailCtnr().find(".form_ctnr form:eq(0)");}
	function getDetailCtnr(){return baseCtnr.find(".detail");}
	function getCurrentTaskId(){return $("#taskId").val();}
	function getCachedTaskId(){return (TASK_ID?TASK_ID:(TASK_ID=$("#taskId").val().trim()));}
	function getCurrentIssueId(){return getCurrentIssue().attr("issueId");}
	function getCurrentIssueName(){return getCurrentIssue().text();}
	function getCurrentIssue(){return baseCtnr.find(".issues .issue.current");}
	
	
	/** 日志记录 */
	function log(msg) {IssueDebug && window.console && console.log(msg);}
