...
function HomeFn() {
	...
	this.initEvents = function() {
		...
		// 初始化 $.confirm 关闭按钮事件
		$("body").undelegate(".projects .items .name", "click").delegate(".projects .items .name", "click", showModulesEvent);

		// 项目右键菜单
		$("body").undelegate(".context_menu", "mouseleave").delegate(".context_menu", "mouseleave", function(){$(this).find(".component").hide(); $(this).hide();});
		$("body").undelegate(".projects .items .module", "contextmenu").delegate(".projects .items .module", "contextmenu", moduleContextMenuEvent);
		$("body").undelegate(".projects .items .module", "click").delegate(".projects .items .module", "click", showTaskListEvent);
		...
		// 任务右键菜单
		$("body").undelegate(".task_list .data_grid .task", "contextmenu").delegate(".task_list .data_grid .task", "contextmenu", taskContextMenuEvent);
		$("body").undelegate(".task_list .data_grid .task", "mousedown").delegate(".task_list .data_grid .task", "mousedown", chooseTaskEvent);
		src.find(".task_context_menu .menu").mouseover(function(){$(this).parent().find(".component").hide();});
	...
	function showTaskIssueEvent() {
		...
		requestUtil.ajax({
			url : "/view.cmd",
			data : {taskId:getCurrTaskId(), url:"task/issue"},
			dataType : "html",
			success : function(rData){jc.setContent(rData);},
	...
	function attachmentsManagerEvent() {
		...
		requestUtil.ajax({
			url : "view.cmd",
			data : {url:"task/attachments_manager", taskId:taskId},
			dataType : "html",
			success : function(rData){
				var jc=$.alert({title:"Task Attachments - "+getCurrTaskName()+getCloseBtn(), content:"", backgroundDismiss:false});
				jc.setContent(rData);
				setTimeout(function(){jc.setDialogCenter();}, 500);
			},
			error : function(){warning();}
		});
	}
	...
	function assigningTaskEvent() {
		...
		requestUtil.ajax({
			url : "view.cmd",
			data : {taskId:taskId, url:"task/assign_task"},
			dataType : "html",
			success : function(data) {
				var jc=$.alert({title:"Assigning Task - "+getCurrTaskName()+getCloseBtn(), content:"", backgroundDismiss:false});
				jc.setContent(data);
				setTimeout(function(){jc.setDialogCenter();}, 500);
			},
			error : function(){warning();}
	...
	function showUploadSqlEvent() {
		...
		requestUtil.ajax({
			url : "view.cmd",
			data : {url:"task/upload_file_view", taskId:getCurrTaskId(), fileType:$this.attr("file-type")},
			dataType : "html",
			success : function(htmlData) {
				$.confirm({title:"Upload Attachment File", content:htmlData, backgroundDismiss:false});
			},
			error : function(){warning();}
		});
	}
	...
	function showTaskDetailsEvent() {
		var taskId = getCurrTaskId;
		var jc = $.alert({title:"Task - "+getCurrTaskName(), content:"", backgroundDismiss:false});
		requestUtil.ajax({
			url : "view.cmd",
			data : {taskId:taskId, url:"task/task_details_view"},
			dataType : "HTML",
			success : function(data) {jc.setContent(data);},
	...
	function taskContextMenuEvent(e) {
		...
		src.find(".task_context_menu").css({top:y, left:x,}).show();
		return false;
	}
	...
	function registerSearchEvent() {
		...
		baseCtnr.find(".search_form .item input.date").focus(function(){var $this=$(this);setTimeout(function(){$this.blur();}, 1000);});
		var dtparams = {format:"yyyy-mm-dd hh:ii", autoclose:true, todayBtn:true, clearBtn:true};
		var createdDateFrom=baseCtnr.find(".search_form .item input[name='createdFromDate']").datetimepicker(dtparams);
		...
		$("body").undelegate(".search_form .item .clear", "click").delegate(".search_form .item .clear", "click", function(){$(this).parent().find(".date").val("");});
		$("body").undelegate(".min_max.min", "click").delegate(".min_max.min", "click", function(){
			var $this=$(this), active=$this.attr("active");
			if (active) {return;}
			$this.attr("active", "active");
			
			var target=$($this.attr("target"));
			var tHeight=target.height() + parseInt(target.css("padding-top")) + parseInt(target.css("padding-bottom"));
			target.attr("org-height", tHeight).slideUp(1000, function(){$this.removeClass("min").addClass("max").removeAttr("active");});
			var dg = baseCtnr.find(".tasks .data_grid");
			dg.animate({height : dg.height() + tHeight}, 1000);
		});
		$("body").undelegate(".min_max.max", "click").delegate(".min_max.max", "click", function(){
			...
			target.slideDown(1000, function(){$this.removeClass("max").addClass("min").removeAttr("active");});
			var dg = baseCtnr.find(".tasks .data_grid");
			dg.animate({height:dg.height()-tHeight}, 1000);
		});
	}
	 ...
	 * @param trigger 触发器
	 * @param valTag 值控件
	 * @param prop 属性 setStartDate|setEndDate
	 */
	function setClickDateScope(trigger, valTag, prop) {trigger.click(function(){$(this).datetimepicker(prop, valTag.val());});}
	...
	function taskMgrEvent() {
		requestUtil.ajax({
			url : "view.cmd",
			data : {moduleId:getActiveModuleId(), url:"task/task_manager"},
			dataType : "html",
			success : function(html){
				if (stringUtil.isEmpty(html, true)) {$.jc.warning("Load task manager window failed."); return;}

				var jc=$.alert({title:"Task Manager - "+getActiveModuleName()+getCloseBtn(), content:"", backgroundDismiss:false, confirm:function(){
	...
	function addTaskEvent() {
		requestUtil.ajax({
			url : "/view.cmd",
			data : {moduleId:getActiveModuleId(), url:"task/task_view"},
			dataType : "html",
			success : function(data) {
				var jc=$.confirm({
					title:"Add Task", 
					...
					cancel:function(){jc.canClose=true;}
				});
				setTimeout(function(){jc.setDialogCenter(true);}, 50);
			},
			error : function() {warning();}
		});
	}
	...
	function submitAddTask() {
		...
		form.validate({
			regex : {
				success : function(error, item){$(item).removeAttr("title").removeClass("ipt_err");},
				failure : function(error, item){jc.canClose=false; $(item).addClass("ipt_err").attr({title:error});}
			}
		});
	...
	function validationName(formData, callback, jc) {
		requestUtil.jsonAjax({
			url : "task/exist.cmd",
			data : {name:formData["name"], moduleId:formData["module.id"]},
			success : function(rData){
				if (rData.flag) {
					if (rData.data) {jc.canClose=true; callback();}
					else {jc.canClose=false; warning(rData.data.msg);}
				}
				else {warning();}
			},
			error : function(){warning();}
	...
	function showTaskListEvent(e) {
		...
		baseCtnr.find(".module").each(function(i,el){$(this).removeClass("active");});
	...
	/**
	 * 获取任务查询条件
	 * @param latestParam true-获取最新查询条件, false-获取缓存的查询条件
	 */
...
//		taskCondition.task = {module:{id:baseCtnr.find(".projects .items .module.active").attr("moduleid")}};
		taskCondition["task.module.id"] = getActiveModuleId();
		return taskCondition;
	}
	
	function getSearchConditionFormCtnr(){return baseCtnr.find(".search_form");}
	function getActiveModuleId(){return getActiveModule().attr("moduleid");}
	function getActiveModuleName(){return getActiveModule().text();}
	function getActiveModule(){return baseCtnr.find(".projects .items .module.active");}
	
	// 模块右键菜单
	function moduleContextMenuEvent(e) {
		var offset=10, x=e.clientX-offset,y=e.clientY-offset;
		src.find(".module_context_menu").css({top:y, left:x,}).show();
		return false;
	}
				...
				if (rData.flag) {
					...
					if (data instanceof Array) {
						...
						if (0 < modulesLen) {
							...
							data.each(function(v){$("<div/>",{html:v["name"], moduleId:v["id"]}).addClass("module").insertAfter($this);});
	...
	function projectSettingsEvent() {
		requestUtil.ajax({
			url : "/view.cmd",
			data : {url:"project/settings"},
			dataType : "html",
			success : function(data) {
				var jc=$.alert({title:"Project Settings" + getCloseBtn(), content:data, backgroundDismiss:false, confirm:initProjects});
				setTimeout(function(){jc.setDialogCenter(true);}, 50);
			},
			error : function() {warning();}
		});
	...
	// 获取 $.confirm 关闭按钮
	function getCloseBtn() {return $("<div/>",{html:"&times;", onClick:"closerEvent(this)"}).addClass("confrim_closer").getHTML();}
	...
	function accountMenuEvent() {
		...
		default:log("Cannot support menu[fn=" + fn + "]");
		}
	}
	...
	function exit() {
		$.confirm({
			...
			backgroundDismiss:false,
			confirm: function(){
				requestUtil.jsonAjax({url:"user/signOut.cmd", success:function(data){
					if (data.flag && data.data){
						location.href = stringUtil.getRealUrl("/index.jsp");
					} else {
						$.confirm({title:"Warning",content:data.message});
					}
				}});
			},
			cancel:function(){
	...
	this.initData = function() {
		...
		$(".trans-tooltip").css({"top":"0", "position":"fixed"});
	}
		...
		requestUtil.jsonAjax({
			...
			success : function(rData){
				...
				if (data) {
					...
					origUsersInModule.each(function(v){usersInModule.push({value:v["key"], text:v["value"]});});
					createdBySl
						.setOptions(usersInModule)
						.prepend($("<option/>",{value:"", html:"ALL", selected:"selected"}))
						.select2();
					
				}
			},
			error : function(r){$.jc.error();}
		});
	}
					...
					if (data instanceof Array) {
						if (0 < data.length) {
							...
							data.each(function(v){
								...
								}).addClass("name").attr({"projectId":v["id"]}).appendTo(itemCtnr);
								$("<img/>",{src:stringUtil.getRealUrl(v["icoPath"])}).prependTo(nameCtnr);
							});
	...
	// 获取当前任务ID
	function getCurrTaskId() {return baseCtnr.find(".data_grid .current_task").attr("taskId");}
	
	// 重新加载任务列表
	function reloadTaskList(){baseCtnr.find("#btnSearch").click();}
	
	// 获取当前任务ID
	function getCurrTaskName() {
		var nameIdx;
		grid.grid.find("thead th").each(function(i,th){if ("name" === $(th).attr("propname")) {nameIdx=i;return false;}});
		return currTaskRow=baseCtnr.find(".data_grid .current_task td:eq('"+nameIdx+"')").text();
	}
	
	// 获取无记录信息容器
	function getNoRecordFoundCtnr() {return $("<div/>").addClass("no_record_found").html(NO_RECORD_FOUND);}
